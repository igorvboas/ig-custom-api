...
3) POST /adicionar-insta/confirmar -> recebe código e tenta concluir login.
...
from app.api.routes import get_collection_service  # <-- usa o pool já inicializado
...
async def ui_start_add_account(
    ...
    try:
        _append_log(onb_id, "Tentando login direto (sem desafio)…")
        ...
        if ok:
            ...
            _append_log(onb_id, "Conta adicionada ao pool" if added else "Conta já estava no pool ou não pôde ser registrada novamente")
            _delete_onb(onb_id)
            body = f"""
<h1>Conta adicionada</h1>
<p class="ok">✅ @{username} pronta para uso.</p>
<div class="row">
  <a class="btn" href="/pool-status">Ver status do pool</a>
  <a class="btn" href="/docs">Abrir Swagger</a>
  <a class="btn" href="/adicionar-insta">Adicionar outra conta</a>
</div>
"""
...
<form method="POST" action="/adicionar-insta/confirmar">
  ...
  <div class="row">
    ...
    <form method="POST" action="/adicionar-insta/cancelar"><input type="hidden" name="onboarding_id" value="{onb_id}" /><button type="submit">Cancelar</button></form>
  </div>
</form>
{_console((_load_onb(onb_id) or {{}}).get("logs", []))}
"""
...
<div class="row">
  <a class="btn" href="/adicionar-insta">Tentar novamente</a>
</div>
{_console((_load_onb(onb_id) or {{}}).get("logs", []))}
"""
...
<a class="btn" href="/adicionar-insta">Voltar</a>
"""
    ...
    try:
        if flow == "TWO_FACTOR":
            # Deflagra estado 2FA se necessário
            try:
                cl.login(username, password)
            except TwoFactorRequired:
                pass
            ok = cl.two_factor_login(code)
            if not ok:
                raise LoginRequired("2FA não aceito")
            _append_log(onboarding_id, "2FA aceito com sucesso")
        else:
            # CHALLENGE: usamos handler que retorna o código informado
            from instagrapi.mixins.challenge import ChallengeChoice
            def _handler(_username: str, choice: "ChallengeChoice"):
                # Apenas registra o método que o IG escolheu internamente
                try:
            ...
            # Tenta logar novamente; a SDK chamará o handler para pegar o código
            ok = cl.login(username, password)
            if not ok:
        ...
        pool = get_collection_service().account_pool
        added = pool.add_account(username, password, proxy)
        _append_log(onboarding_id, "Conta adicionada ao pool" if added else "Conta já estava no pool ou não pôde ser registrada novamente")
...
<form method="POST" action="/adicionar-insta/confirmar">
  ...
  <div class="row">
    <button type="submit">Confirmar</button>
    <form method="POST" action="/adicionar-insta/cancelar"><input type="hidden" name="onboarding_id" value="{onboarding_id}" /><button type="submit">Cancelar</button></form>
  </div>
</form>
{_console(logs)}
"""
...
<a class="btn" href="/adicionar-insta">Voltar ao início</a>
"""
